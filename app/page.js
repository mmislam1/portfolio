...
export default function Home() {
  ...
  const projects = [
    {
      title: "Lose to gain",
      desc: " Lets users create diet plans and generate and send diet charts to user email addresses.",
    ...
    },
    {
      ...
      tools: ["PHP", "MySQL"],
      link: "http://github.com/mmislam1/MetroRail",
    },
  ];
      ...
      <div className=" grid items-center justify-center p-8 font-semibold text-lg lg:px-72">
        <div className=" grid items-center justify-around grid-cols-1">
          ...
          <div className="flex flex-row m-10 items-center justify-center">
            <button className="bg-amber-400 border-2 border-amber-400 text-slate-900 text-2xl px-8 p-2 font-semibold rounded hover:bg-slate-900 hover:text-amber-400 m-5 lg:m-10">
              <a href="https://drive.google.com/file/d/1PN9v4fXd6DuopgR9tGupJshRp0AhurtF/view?usp=sharing">
                Resume
              </a>
            </button>

            <a href="https://github.com/mmislam1" className="m-5 lg:m-10">
              <FontAwesomeIcon
                icon={faGithub}
            ...
            >
              <FontAwesomeIcon
                icon={faLinkedin}
                className="animate-colorChange hover:text-orange-600 text-5xl"
              ></FontAwesomeIcon>
            </a>
          </div>

          <h2
            className="text-amber-400 font-semibold text-4xl mt-10 m-auto"
            id="skills"
          >
            SKILLS
          </h2>
          ...
          <div className=" grid grid-cols-1 items-center justify-between gap-6 p-5 my-6 border-2 border-slate-500 bg-slate-700 rounded-xl md:grid-cols-2 xl:grid-cols-3">
            {skills.map((skill) => {
              ...
              return (
                <div
                  key={skill.title}
                  className="grid grid-cols-1 items-center justify-center bg-slate-900 border-2 border-slate-500 p-5 rounded-md"
                >
                  <h2 className="text-amber-400 font-semibold md:text-xl xl:text-2xl mb-4 m-auto">
                    {skill.title}
                  </h2>
          ...
          <div className="grid grid-cols-1 items-center justify-between gap-6 py-5 my-4  ">
            {projects.map((project) => {
              ...
              return (
                <div
                  key={project.title}
                  className=" bg-slate-900 border-2 border-amber-400 p-5 rounded-md"
                >
                  <div className="flex flex-row mb-4 gap-4">
                    <h4 className="text-amber-400 text-3xl">
                      {project.title}
                    </h4>
                    <a
                      href={project.link}
                    ...
                    >
                      <FontAwesomeIcon
                        icon={faGithub}
                        className=" text-3xl animate-colorChange hover:text-orange-600"
                      ></FontAwesomeIcon>
                    </a>
                  </div>
                  <p className="regulartext mb-4 bg-slate-700 p-2 rounded-md">
                    {project.desc}
                  </p>
                  <div className="flex flex-row flex-wrap">{tools}</div>
                </div>
              );
            })}
          </div>
          ...
          <div className="contact section grid grid-cols-1  ">
            ...
            <form
              className="mt-10 grid grid grid-cols-1 gap-4 items-center justify-center lg:grid-cols-2  "
              action="https://formspree.io/f/xzzppron"
              method="POST"
            >
              <input
                className="p-2 rounded-md text-xl border-2 border-amber-400 font-semibold lg:col-start-1 bg-slate-900 text-amber-400 lg:col-span-1"
                type="text"
                name="name"
                placeholder="Name"
              />
              ...
              <div className="p-6 grid items-center lg:col-start-1">
                <button
                  className="bg-amber-400 border-2 border-amber-400 text-slate-900 text-2xl px-8 p-2 font-semibold rounded hover:bg-slate-900 hover:text-amber-400 "
                  type="submit"
                >
                  Send
                </button>
